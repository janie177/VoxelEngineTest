...
public class BlockRenderer
{
	...
	public BlockRenderer(DefaultShader shader)
	{
		this.shader = shader;
	}
	...
	private void bindModel(TexturedModel model, boolean transparent)
	{
		...
		//Supply the texture ID. This was supplied to OpenGL before, and will be acquired by ID since the same ID is in the model class. OpenGL will
		//then know which texture has to be drawn.
		GL11.glBindTexture(GL11.GL_TEXTURE_2D, texture.getId());
	}


	public void renderBlocks(List<Block> blocks, BlockType type)
	{
		if(blocks.size() < 1)return;

		TexturedModel model = type.getTexturedModel();

		//Bind the model. Set to transparent if the texture is transparent.
		bindModel(model, type.isTransparent());

		int vertexCount = model.getModel().getVertexCount();
		for(Block b : blocks)
		{
			//Create the transformation matrix which determines how an entity looks.
			Matrix4f matrix = MathUtil.createTransformationMatrix(b.getLocation(), 0, 0, 0, 1);
